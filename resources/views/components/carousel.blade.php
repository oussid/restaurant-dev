<div class="home-carousel">
    <div class="home-carousel-slide active" style="background-image: url({{asset('images/slide2.jpg')}}); ">
        <div class="home-carousel-slide-text">
            <h2>Flat 20% Discount</h2>
            <h1>Stay Home & Delivered Your Foods</h1>
        </div>
        <button class="home-carousel-slide-btn">
            Explore <i class="fa-solid fa-arrow-right"></i>
        </button>
    </div>
    <div class="home-carousel-slide" style="background-image: url({{asset('images/slide3.jpg')}}); ">
        <div class="home-carousel-slide-text">
            <h2>Flat 20% Discount</h2>
            <h1>Stay Home & Delivered Your Foods</h1>
        </div>
        <button class="home-carousel-slide-btn">
            Explore <i class="fa-solid fa-arrow-right"></i>
        </button>
    </div>
    <div class="home-carousel-slide" style="background-image: url({{asset('images/slide4.jpg')}}); ">
        <div class="home-carousel-slide-text">
            <h2>Flat 20% Discount</h2>
            <h1>Stay Home & Delivered Your Foods</h1>
        </div>
        <button class="home-carousel-slide-btn">
            Explore <i class="fa-solid fa-arrow-right"></i>
        </button>
    </div>

    <button class="home-carousel-next-btn"><i class="fa-solid fa-chevron-right"></i></button>
    <button class="home-carousel-prev-btn"><i class="fa-solid fa-chevron-left"></i></button>
</div>
<script src="js/carousel.js"></script>
